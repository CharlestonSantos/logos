...
//   console.log('✦ Importação automática — continua do banco\n')

//   let [version] = await db`
//     SELECT id FROM bible_versions WHERE code=${VERSION_CODE}
//   `
...
//     const abr = BOOK_MAP[book.code]
//     if (!abr) {
//       console.warn(`⚠ Sem mapeamento para ${book.code}`)
//       continue
//     }

//     // último capítulo salvo
//     const [last] = await db`
//       SELECT MAX(chapter) AS last
//       FROM bible_verses
//       WHERE version_id=${version.id}
//       AND book_id=${book.id}
//     `

//     const startChapter = (last?.last || 0) + 1
//     const totalChapters = book.chapter_count   // ✅ CORRETO

//     if (startChapter > totalChapters) {
//       console.log(`📖 ${book.code.padEnd(5)} ✓ já completo`)
//       continue
//     }

//     process.stdout.write(
//       `📖 ${book.code.padEnd(5)} iniciando cap ${startChapter} `
//     )
...
//         ${db(rows, 'version_id','book_id','chapter','verse','text')}
//         ON CONFLICT (version_id,book_id,chapter,verse) DO NOTHING
//       `

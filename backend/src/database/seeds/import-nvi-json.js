...
async function main() {
  ...
  // ── 4. Limpa NVI só se não houver dados (primeira execução) ──
  const [{ count: existing }] = await db`SELECT COUNT(*)::int as count FROM bible_verses WHERE version_id = ${vId.NVI}`
  if (existing <= 1) {
    ...
    console.log(`✓ NVI já tem ${existing} versículos — mantendo dados existentes\n`)
  }

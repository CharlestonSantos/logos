    ...
    <aside class="notes-sidebar">
      ...
      <div class="tags-row" v-if="tags.length">
        <button
          ...
          @click="filters.tag = ''; loadNotes()"
        >Todas</button>
        <button
          v-for="tag in tags" :key="tag.id"
          class="tag-pill"
          :class="{ active: filters.tag === tag.name }"
          @click="filters.tag = tag.name; loadNotes()"
        >{{ tag.name }} <span class="tag-count">{{ tag.noteCount }}</span></button>
      </div>
    ...
    <main class="notes-editor" v-if="editing">
      ...
      <div class="editor-tags">
        <div class="tags-input-row">
          <span
            v-for="tag in editingTags" :key="tag"
            class="editor-tag"
          >
            {{ tag }}
...
.notes-page {
  ...
  background: #F9F6F0; font-family: 'DM Sans', sans-serif; color: #1C1A17;
}

/* ── Sidebar ─────────────────────────────────────────────── */
.notes-sidebar {
  width: 300px; flex-shrink: 0;
  background: #FDFBF8; border-right: 1px solid rgba(28,26,23,0.1);
...
.notes-title { font-family: 'Lora', serif; font-size: 1rem; font-weight: 600; }
.btn-new {
  padding: 0.35rem 0.75rem; background: #1C1A17; color: #F9F6F0;
  border: none; border-radius: 6px; cursor: pointer; font-size: 0.78rem;
  transition: background 0.2s;
}
.btn-new:hover { background: #B8965A; }

.search-box { padding: 0 1.25rem 0.75rem; }
.search-box input {
  width: 100%; padding: 0.6rem 0.85rem;
  background: #F9F6F0; border: 1px solid rgba(28,26,23,0.1);
  border-radius: 8px; font-family: 'DM Sans', sans-serif;
  font-size: 0.82rem; color: #1C1A17; outline: none;
  transition: border-color 0.2s;
}
.search-box input:focus { border-color: #B8965A; }
.search-box input::placeholder { color: #9C9590; }
...
.tag-pill {
  ...
  cursor: pointer; color: #4A4540; transition: all 0.15s;
  display: flex; align-items: center; gap: 0.3rem;
}
.tag-pill.active { background: #1C1A17; color: #F9F6F0; border-color: #1C1A17; }
...
.note-item:hover  { background: rgba(184,150,90,0.06); }
.note-item.active { background: rgba(184,150,90,0.12); }

.note-item-header { display: flex; justify-content: space-between; align-items: center; margin-bottom: 0.35rem; }
.note-ref { font-size: 0.68rem; font-weight: 700; color: #B8965A; }
.note-ref.free { color: #8A9E8C; }
.note-date { font-size: 0.65rem; color: #9C9590; }
.note-preview {
  font-size: 0.8rem; color: #4A4540; line-height: 1.45;
  display: -webkit-box; -webkit-line-clamp: 2; -webkit-box-orient: vertical; overflow: hidden;
  margin-bottom: 0.4rem;
}
.note-tags-row { display: flex; gap: 0.3rem; flex-wrap: wrap; }
.note-tag {
  font-size: 0.6rem; padding: 0.1rem 0.4rem;
  background: rgba(138,158,140,0.15); color: #8A9E8C; border-radius: 20px;
}

.notes-empty { text-align: center; padding: 3rem 1.5rem; color: #9C9590; font-size: 0.85rem; }
.notes-empty .empty-icon { font-size: 2rem; margin-bottom: 0.75rem; }
.btn-new-empty {
  margin-top: 1rem; padding: 0.55rem 1.25rem;
  background: #1C1A17; color: #F9F6F0; border: none;
  border-radius: 8px; cursor: pointer; font-size: 0.82rem;
  transition: background 0.2s;
}
.btn-new-empty:hover { background: #B8965A; }

.notes-loading { text-align: center; padding: 2rem; color: #9C9590; font-size: 0.82rem; }

.pagination { padding: 0.75rem 1.25rem; border-top: 1px solid rgba(28,26,23,0.06); }
.btn-more {
  width: 100%; padding: 0.55rem; background: none;
  border: 1px solid rgba(28,26,23,0.12); border-radius: 8px;
  cursor: pointer; font-size: 0.78rem; color: #4A4540; transition: all 0.2s;
}
.btn-more:hover { border-color: #B8965A; color: #B8965A; }
...
.editor-verse-ref { display: flex; align-items: baseline; gap: 0.75rem; flex-wrap: wrap; }
.ref-badge {
  font-size: 0.72rem; font-weight: 700; color: #B8965A;
  letter-spacing: 0.08em; text-transform: uppercase;
}
.ref-badge.free { color: #8A9E8C; }
.ref-text {
  font-family: 'Lora', serif; font-size: 0.85rem; font-style: italic;
  color: #4A4540; max-width: 500px;
  display: -webkit-box; -webkit-line-clamp: 2; -webkit-box-orient: vertical; overflow: hidden;
}

.editor-actions { display: flex; gap: 0.5rem; flex-shrink: 0; margin-left: 1rem; }
.btn-delete {
  padding: 0.5rem 0.75rem; background: none;
  border: 1px solid rgba(196,122,122,0.3); border-radius: 8px;
  cursor: pointer; font-size: 0.85rem; color: #C47A7A; transition: all 0.2s;
}
.btn-delete:hover { background: rgba(196,122,122,0.08); }
.btn-save {
  padding: 0.5rem 1.25rem; background: #1C1A17; color: #F9F6F0;
  border: none; border-radius: 8px; cursor: pointer;
  font-size: 0.82rem; font-weight: 500; transition: background 0.2s;
}
.btn-save:hover:not(:disabled) { background: #B8965A; }
...
.editor-tag button {
  ...
  color: #B8965A; font-size: 0.85rem; line-height: 1; padding: 0;
  opacity: 0.7;
}
...
.tag-input {
  ...
  color: #1C1A17; min-width: 120px;
}
.tag-input::placeholder { color: #9C9590; }

/* Textarea */
.note-textarea {
  flex: 1; padding: 2rem;
  border: none; outline: none; resize: none;
  background: #F9F6F0; font-family: 'Lora', serif;
  font-size: 1rem; line-height: 1.8; color: #1C1A17;
}
.note-textarea::placeholder { color: rgba(28,26,23,0.25); font-style: italic; }

.editor-footer {
  display: flex; justify-content: space-between; align-items: center;
  padding: 0.6rem 2rem;
  background: #FDFBF8; border-top: 1px solid rgba(28,26,23,0.06);
  font-size: 0.72rem; color: #9C9590;
}
.save-status { color: #4A8A4C; }

/* ── Estado vazio ────────────────────────────────────────── */
.notes-empty-main {
  flex: 1; display: flex; align-items: center; justify-content: center;
}
.empty-state { text-align: center; color: #9C9590; }
.empty-icon-lg { font-size: 3rem; margin-bottom: 1rem; }
.empty-title { font-family: 'Lora', serif; font-size: 1.3rem; color: #4A4540; margin-bottom: 0.5rem; }
.empty-sub { font-size: 0.85rem; margin-bottom: 1.5rem; }
.btn-new-lg {
  padding: 0.75rem 1.75rem; background: #1C1A17; color: #F9F6F0;
  border: none; border-radius: 8px; cursor: pointer;
  font-size: 0.88rem; font-weight: 500; transition: background 0.2s;
}
.btn-new-lg:hover { background: #B8965A; }

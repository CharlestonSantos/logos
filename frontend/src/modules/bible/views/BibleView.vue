  ...
  <div class="bible-view">
    ...
    <aside class="sidebar" :class="{ open: sidebarOpen }">
      ...
      <div class="testament-tabs">
        <button
          ...
          class="testament-tab"
          :class="{ active: testament === t }"
          @click="testament = t"
        >{{ t === 'AT' ? 'Antigo' : 'Novo' }}</button>
      </div>

      <!-- Lista de livros -->
      <div class="book-list">
        <button
          v-for="book in filteredBooks" :key="book.code"
          class="book-item"
          :class="{ active: selectedBook?.code === book.code }"
      ...
      <div v-if="selectedBook" class="chapter-selector">
        ...
        <div class="chapter-grid">
          <button
            v-for="n in selectedBook.chapterCount" :key="n"
            class="chapter-btn"
    ...
    <main class="reader">
      ...
      <div class="highlight-toolbar" v-if="selectedVerseId" ref="toolbar">
        ...
        <button
          v-for="color in highlightColors" :key="color.key"
          class="color-btn"
          :style="{ background: color.bg }"
          :title="color.label"
          @click="applyHighlight(color.key)"
        ></button>
      ...
      <div class="chapter-content" v-else-if="chapter" :style="{ fontSize: fontSize + 'px' }">
        ...
        <div class="chapter-nav">
          <button
            v-if="chapter.navigation.prev"
            class="nav-btn"
            @click="loadChapter(chapter.navigation.prev.chapter)"
          >← Capítulo {{ chapter.navigation.prev.chapter }}</button>

          <div class="nav-spacer"></div>

          <button
            v-if="chapter.navigation.next"
            class="nav-btn nav-btn-next"
            @click="loadChapter(chapter.navigation.next.chapter)"
          >Capítulo {{ chapter.navigation.next.chapter }} →</button>
        </div>
      </div>
...
.testament-tab {
  flex: 1; padding: 0.4rem; border: 1px solid rgba(28,26,23,0.12);
  border-radius: 6px; background: transparent; cursor: pointer;
  font-size: 0.78rem; font-weight: 500; color: #4A4540;
  transition: all 0.2s;
}
.testament-tab.active {
  background: #1C1A17; color: #F9F6F0; border-color: #1C1A17;
}
...
.book-abbr {
  font-family: 'DM Sans', monospace; font-size: 0.68rem; font-weight: 600;
  color: #B8965A; width: 30px; flex-shrink: 0;
}
...
.chapter-label {
  ...
  color: #9C9590; margin-bottom: 0.75rem; font-weight: 600;
}
.chapter-grid {
  display: grid; grid-template-columns: repeat(6, 1fr); gap: 0.35rem;
}
.chapter-btn {
  padding: 0.4rem; border: 1px solid rgba(28,26,23,0.1);
  border-radius: 6px; background: none; cursor: pointer;
  font-size: 0.75rem; color: #4A4540; transition: all 0.15s;
}
...
.menu-btn span {
  ...
  background: #1C1A17; border-radius: 2px; transition: all 0.2s;
}
...
.version-select {
  padding: 0.35rem 0.6rem; border: 1px solid rgba(28,26,23,0.12);
  border-radius: 6px; background: #F9F6F0; font-family: 'DM Sans', sans-serif;
  font-size: 0.78rem; color: #1C1A17; cursor: pointer; outline: none;
}

.font-controls { display: flex; gap: 0.25rem; }
.font-controls button {
  padding: 0.3rem 0.6rem; border: 1px solid rgba(28,26,23,0.12);
  border-radius: 6px; background: none; cursor: pointer;
  font-size: 0.75rem; color: #4A4540; transition: all 0.2s;
}
.font-controls button:hover { border-color: #B8965A; color: #B8965A; }

.notes-toggle {
  padding: 0.35rem 0.6rem; border: 1px solid rgba(28,26,23,0.12);
  border-radius: 6px; background: none; cursor: pointer; font-size: 0.9rem;
  transition: all 0.2s;
}
...
.toolbar-note-btn, .toolbar-ref-btn, .toolbar-close {
  ...
  border-radius: 6px; cursor: pointer; padding: 0.25rem 0.5rem;
  font-size: 0.85rem; color: white; transition: background 0.2s;
}
...
.chapter-num {
  font-family: 'Lora', serif; font-size: 3rem; font-weight: 300;
  color: rgba(184,150,90,0.4); line-height: 1;
}
...
.verse-num {
  font-size: 0.65em; font-weight: 700; color: #B8965A;
  min-width: 24px; flex-shrink: 0; padding-top: 0.1em;
  user-select: none;
}
...
.note-indicator {
  ...
  font-size: 0.45rem; color: #B8965A;
}

/* Skeleton loading */
.skeleton {
  height: 1.2rem; background: rgba(28,26,23,0.06);
...
.nav-btn {
  ...
  background: #F9F6F0; border: 1px solid rgba(28,26,23,0.12);
  border-radius: 8px; cursor: pointer; font-family: 'DM Sans', sans-serif;
  font-size: 0.82rem; color: #4A4540; transition: all 0.2s;
}
...
.panel-header button {
  ...
  color: #9C9590; font-size: 0.9rem;
}

/* Editor de nota */
.note-verse-ref {
  font-size: 0.72rem; color: #B8965A; letter-spacing: 0.08em;
  text-transform: uppercase; margin-bottom: 0.5rem; font-weight: 600;
}
.note-textarea {
  width: 100%; border: 1px solid rgba(28,26,23,0.12);
  border-radius: 8px; padding: 0.75rem; resize: vertical;
  font-family: 'DM Sans', sans-serif; font-size: 0.85rem;
  background: #F9F6F0; color: #1C1A17; outline: none;
  transition: border-color 0.2s;
}
.note-textarea:focus { border-color: #B8965A; }
.note-actions { display: flex; gap: 0.5rem; margin-top: 0.75rem; }
.btn-save {
  flex: 1; padding: 0.55rem; background: #1C1A17; color: #F9F6F0;
...
.btn-cancel {
  ...
  border-radius: 6px; cursor: pointer; font-size: 0.82rem; color: #4A4540;
}

/* Lista de notas */
.notes-list-title {
  font-size: 0.68rem; text-transform: uppercase; letter-spacing: 0.1em;
  color: #9C9590; margin: 1.25rem 0 0.75rem; font-weight: 600;
}
.note-card {
  background: #F9F6F0; border-radius: 8px;
...
.note-card-edit {
  ...
  background: none; border: none; cursor: pointer; font-size: 0.75rem; opacity: 0.5;
}
.note-card-edit:hover { opacity: 1; }

/* Referências */
.refs-verse-ref {
  font-size: 0.72rem; color: #B8965A; font-weight: 600;
...
@media (max-width: 1024px) {
  .sidebar {
    ...
    transform: translateX(-100%); box-shadow: 4px 0 24px rgba(0,0,0,0.1);
  }

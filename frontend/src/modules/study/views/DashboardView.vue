      ...
      <div class="dash-main">
        ...
        <div class="section" v-if="recentReading.length">
          ...
          <div class="recent-grid">
            <RouterLink
              v-for="item in recentReading" :key="item.ref"
              to="/leitura"
              class="recent-card"
            >
          ...
          <div class="section-header">
            ...
            <div class="testament-pills">
              <button
                ...
                class="pill" :class="{ active: quickTestament === t }"
                @click="quickTestament = t"
              >{{ t === 'AT' ? 'Antigo Testamento' : 'Novo Testamento' }}</button>
            </div>
          </div>
          <div class="books-grid">
            <RouterLink
              v-for="book in quickBooks" :key="book.code"
              to="/leitura"
              class="book-card"
            >
...
const recentNotes = ref([
  { id: 1, ref: 'João 3:16', text: 'O amor de Deus manifestado na entrega do Filho...' },
  { id: 2, ref: 'Salmos 23:1', text: 'O Senhor como pastor — provisão completa.' },
])

// Livros para acesso rápido
const allBooks = [
  // AT
...
.greeting-sub { font-size: 0.8rem; color: #9C9590; letter-spacing: 0.05em; }
.greeting-name {
  font-family: 'Lora', serif; font-size: 2rem; font-weight: 600;
  color: #1C1A17; line-height: 1.1;
}
.dash-date { font-size: 0.8rem; color: #9C9590; text-transform: capitalize; }
...
.verse-card::before {
  ...
  font-size: 6rem; color: rgba(184,150,90,0.12);
  font-family: 'Lora', serif; line-height: 1;
}
.verse-card-label {
  font-size: 0.68rem; letter-spacing: 0.15em; text-transform: uppercase;
  color: #B8965A; margin-bottom: 1rem; font-weight: 600;
}
.verse-card-text {
...
.verse-card-btn {
  ...
  background: rgba(184,150,90,0.2); color: #D4B07A;
...
.section-title {
  ...
  letter-spacing: 0.12em; text-transform: uppercase; color: #9C9590;
  margin-bottom: 1rem;
}
...
.pill {
  ...
  cursor: pointer; color: #4A4540; transition: all 0.2s;
}
...
.book-card {
  background: #FDFBF8; border: 1px solid rgba(28,26,23,0.08);
  border-radius: 10px; padding: 0.9rem 0.75rem; text-decoration: none;
...
/* Progress bar */
.progress-bar {
  flex: 1; height: 4px; background: rgba(28,26,23,0.08);
...
.stats-title {
  ...
  text-transform: uppercase; color: #9C9590; margin-bottom: 1.25rem;
}
...
.plan-today-label {
  ...
  text-transform: uppercase; color: #8A9E8C; margin-bottom: 0.5rem;
}
